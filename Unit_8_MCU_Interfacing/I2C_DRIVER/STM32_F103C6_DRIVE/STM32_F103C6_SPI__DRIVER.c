...
void MCAL_SPI_INIT(SPI_TypeDef* SPIx,SPI_Config_t* SPI_Config)
{
	...
	TempReg_CR1 |= SPI_Config->SPI_Baud_Rate_Prescaler;
...
void SPI1_IRQHandler(void)
{
	...
	irq_scr.TXE   = ( (SPI1->SPI_SR & (SPI_SR_TXE)>>1) );
	irq_scr.RXNE  = ( (SPI1->SPI_SR & (SPI_SR_RXNE)>>0) );
	irq_scr.ERROR = ( (SPI1->SPI_SR & (1<<4)>>4) );
...
{
	struct S_IRQ_SRC irq_src;
	irq_scr.TXE   = ( (SPI2->SPI_SR & (SPI_SR_TXE)>>1) );
	irq_scr.RXNE  = ( (SPI2->SPI_SR & (SPI_SR_RXNE)>>0) );
	irq_scr.ERROR = ( (SPI2->SPI_SR & (1<<4)>>4) );
