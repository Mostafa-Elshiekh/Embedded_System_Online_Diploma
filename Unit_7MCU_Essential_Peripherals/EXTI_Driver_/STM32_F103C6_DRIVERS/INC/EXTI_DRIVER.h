...
typedef struct
{
	uint8_t            EXTI_INPUT_LINE_NUMBER;
...
typedef struct
{
	EXTI_GPIO_Mapping_t EXTI_PIN;    // Specifies the GPIO EXTI GPIO Mapping.
	//This parameter must be set based on @Ref EXTI_define

	uint8_t             TRIGGRE_CASE;  // Specifies the TRIGGRE_CASE Rising Or Falling Trigger.
	//This parameter must be set based on @Ref EXTI_TRIGGRE_define

	uint8_t             IRQ_EN;       // Specifies Enable Or Disable EXTI IRQ_EN (This Enable Mask In EXTI && NVIC Interrupt Controller)
	//This parameter must be set based on @Ref EXTI_IRQ_EN_define
...
//@Ref EXTI_TRIGGRE_define
#define EXTI_TRIGGRE_RISING                0
#define EXTI_TRIGGRE_FALLING               1
#define EXTI_TRIGGRE_RISING_AND_FALLING    2
